...
public class Visita {
	...
	//metodo que ordene los monumentos de la antiguo al mas actual
	/*
	...
	*/
	...
	public void ordenarMonumentoPorEdad () {
		for (int i = 0; i < monumento.length-1; i++) {
			for (int j = i+1; j < monumento.length; j++) {
				if (monumento[j].getAnyosAntiguedad() < monumento[i].getAnyosAntiguedad()) {
					Monumento anyosMonumento = monumento[i];
					monumento[i] = monumento[j];
					monumento[j] = anyosMonumento;
				}
			}
		}
	}
