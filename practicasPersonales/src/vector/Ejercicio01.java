...
public class Ejercicio01 {
	...
	static void invertirModo1 (int [] vector) {//invierte el orden || con char ordena numero de 1 a mayor

		for (int i=0; i<vector.length-1; i++) { //ORDENAR (IGUAL)
	...
	public static int[] invetir (int[] numero) {
		int[] vector = new int[numero.length];
		int indice = 0;
		for (int i = numero.length-1; i>=0; i--) {
			vector[indice++] = numero[i];
		}
		return vector;
	}
